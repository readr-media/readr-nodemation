...
export type CmsOutputAudioTargetField = "title" | "description" | "audioFile";

export type CmsAudioFieldMapping = {
  id: string;
  sourceField: string;
  targetField: CmsOutputAudioTargetField;
};

export type CmsOutputAudioNodeData = {
  title: string;
  cmsConfigId: string;
  cmsName: string;
  cmsList: string;
  cmsAudioFileIds: string;
  mappings: CmsAudioFieldMapping[];
  mode: "create";
};
export type CmsOutputAudioNodeType = Node<CmsOutputAudioNodeData, "cmsOutputAudio">;

const handleClass =
  "!h-3 !w-3 !rounded-full !border !border-[#d4d3cc] !bg-white shadow-[0px_2px_6px_rgba(0,0,0,0.1)]";

const CmsOutputAudioNode = ({ data }: NodeProps<CmsOutputAudioNodeType>) => {
...
export default memo(CmsOutputAudioNode);
